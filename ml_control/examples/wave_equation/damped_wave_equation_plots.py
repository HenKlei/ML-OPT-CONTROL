...
animate_solution(x_opt[:, :N//2], ylim=(np.min(x_opt) * 1.1, np.max(x_opt) * 1.1), title="Optimal solution")
...
plot_final_time_solutions([x0, x_opt[-1], xT], labels=["Initial state", "Optimal state", "Target state"], show_plot=False, ax=axs[2])
